...
uint8_t spGOOD[]      PROGMEM = {
  0x0A,0x28,0xCD,0x34,0x20,0xD9,0x1A,0x45,0x74,0xE4,0x66,0x24,0xAD,0xBA,0xB1,0x8C,0x9B,0x91,0xA5,0x64,0xE6,0x98,0x21,0x16,0x0B,0x96,0x9B,0x4C,0xE5,0xFF,0x01};
uint8_t spMORNING[]   PROGMEM = {
...
void loop() {
 ...
 while (digitalRead (BUTTON) == HIGH); // wait for button to pressed
 ...
 delay(100); 
  ...
  if(button_delay > 5){ //no need to wait for user to release
  ...
  sayNumber(RTC.minute);
  if (heure<12) {
    voice.say(spA_M_);
  }
  else voice.say(spP_M_);
